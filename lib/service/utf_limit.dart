...
class Utf8LengthLimitingTextInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    if (maxLength != null && maxLength > 0 && bytesLength(newValue.text) > maxLength ||
        newValue.text.length > 10 || (bytesLength(newValue.text) + newValue.text.length > 20)) {
      // If already at the maximum and tried to enter even more, keep the old value.
      if (bytesLength(oldValue.text) == maxLength || newValue.text.length >= 10 || (bytesLength(newValue.text) + newValue.text.length >= 20)) {
        return oldValue;
      }
